...
public class GetPaySlipRequetsHandler : IRequestHandler<GetPaySlipRequets, Domain.Entities.PaySlip>
{
    ...
    {
        //get Attendance by ?
        var item = _applicationDbContext.Get<Domain.Entities.PaySlip>().Include(x => x.EmployeeContract.ApplicationUser).Where(x => x.IsDeleted == false && x.Id== request.Id).AsNoTracking().FirstOrDefault();
